...
class CoreScanner(object):
	...
	scan_type_options = { "PingScan":"-n -sn -T5", "PortScan":"-n -Pn -T5 --open", "OsScan":"-n -Pn -O -T5", "ScriptScan":"-n -Pn -T5" }
		...
		nmap_options = "{0} {1} -iL {2}".format(CoreScanner.scan_type_options[self.scan_type], Core.nmap_optimize, self.ip_file) if nmap_optimize else "{0} -iL {1}".format(CoreScanner.scan_type_options[self.scan_type], self.ip_file)
	...
	def run(self):
		...
		cmd = "{0} {1} -oA {2}".format(self.proc_cmd, self.scan_options, self.__output_file) if self.scan_type in ( "PortScan", "ScriptScan") else "{0} -oA {1}".format(self.proc_cmd, self.__output_file)
